...
import { api, type PlayerIndexRow, type PlayerSeason, type ScoutingReport, type SearchHit, type SimilarPlayer } from '../lib/api'
    ...
    <div className="space-y-4">
      ...
      {latest && playerSeasons && (
        <>
          ...
          <Card title="AI scouting report">
            {report ? (
              <>
                <p className="text-sm leading-relaxed whitespace-pre-wrap">{report.report}</p>
                <p className="mt-3 text-xs text-[var(--text-muted)]">
                  Generated by {report.provider} · {report.model}, grounded only in the
                  warehouse data shown on this page.
                </p>
              </>
            ) : (
              <div className="space-y-2">
                <p className="text-sm text-[var(--text-secondary)]">
                  Generate a written scout's view of {selected!.player_name}, built strictly
                  from the stats in this warehouse.
                </p>
                {reportError && <p className="text-sm text-[var(--series-2)]">⚠ {reportError}</p>}
                <button
                  className="card cursor-pointer bg-[var(--series-1)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
                  disabled={reportLoading}
                  onClick={async () => {
                    setReportLoading(true)
                    setReportError('')
                    try {
                      setReport(await api.scoutingReport(selected!.player_id))
                    } catch (e) {
                ...
                >
                  {reportLoading ? 'Writing report…' : 'Generate scouting report'}
                </button>
              </div>
            )}
          </Card>
